...
import { makeHandler } from "../../mcp/tools/complete-integration.js";
...
describe("complete_integration tool", () => {
  let tmpData;
  let pluginData;

  beforeEach(() => {
    tmpData = fs.mkdtempSync(path.join(os.tmpdir(), "blazer-complete-"));
    pluginData = new PluginData(tmpData);
    // Pre-populate active session
    pluginData.writeActiveSession({ journey_id: "j1", session_id: "s1", product_id: "datadog" });
  });
  ...
  it("calls POST /journeys/complete with args", async () => {
    ...
    assert.strictEqual(client._lastCall.path, "/journeys/complete");
