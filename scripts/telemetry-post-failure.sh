...
echo "$INPUT" | jq -c --arg ts "$TIMESTAMP" --arg jid "$JOURNEY_ID" --arg sid "$SESSION_ID" \
  --argjson dur "$DURATION_MS" --arg phase "$PHASE" \
  '{event: "post_tool_use_failure", journey_id: $jid, session_id: $sid, tool_name: .tool_name, error: .tool_response.error, timestamp: $ts, success: false, duration_ms: $dur, phase: $phase}' \
